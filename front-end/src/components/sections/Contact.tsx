...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (web3FormsKey) {
        // Web3Forms — works on static exports (no server needed)
        const response = await fetch("https://api.web3forms.com/submit", {
          method: "POST",
          headers: { "Content-Type": "application/json", Accept: "application/json" },
          body: JSON.stringify({
            access_key: web3FormsKey,
            subject: `[manibuildsai.com] ${formData.subject} — from ${formData.name}`,
        ...
        if (!response.ok) {
          // Last resort: open mailto so the message is never lost
          const body = encodeURIComponent(
            `Name: ${formData.name}\nEmail: ${formData.email}\n\n${formData.message}`
          );
          const subject = encodeURIComponent(`[manibuildsai.com] ${formData.subject}`);
